        ...
        for indices in combinations(range(len(FEATURES)), size):
            ...
            for shift in shifts_to_use:
                ...
                if new_separation < (ORIGINAL_SEPARATION * SEPARATION_REDUCTION_THRESHOLD):
                    # E.g.: pert_corr3fn_by1p2s.yml for features 1,2,3 (any indices, always 'n' for class 0)
                    filename = f"pert_corr{len(names)}f{CLASS_NAME_IN_FILENAME}_by{format_val(shift)}s.yml"
                    config = {
